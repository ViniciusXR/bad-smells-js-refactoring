...
import { ReportGeneratorRefactored } from '../src/ReportGeneratorRefactored.js';
...
const generators = [
  { name: 'ReportGenerator', GeneratorClass: ReportGenerator },
  { name: 'ReportGeneratorRefactored', GeneratorClass: ReportGeneratorRefactored },
];

const runReportGeneratorTests = (GeneratorClass, name) => {
  describe(`${name} (Rede de Segurança)`, () => {
    let generator;

    beforeEach(() => {
      generator = new GeneratorClass(mockDb);
    });
...
generators.forEach(({ name, GeneratorClass }) => {
  runReportGeneratorTests(GeneratorClass, name);
});